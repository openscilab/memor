...
from .params import Role, RenderFormat, LLM_MODEL
...
class Response:
    ...
    >>> response = Response(message="Hello!", score=0.9, role=Role.ASSISTANT, temperature=0.5, model=LLM_MODEL.GPT_4)
    >>> response.message
    'Hello!'
    """
    ...
    def update_model(self, model: LLM_MODEL) -> None:
        """
        Update the agent model.

        :param model: model
        """
        if not isinstance(model, LLM_MODEL):
    ...
    def from_json(self, json_object: Union[str, Dict[str, Any]]) -> None:
        ...
        try:
            ...
            self._model = LLM_MODEL(loaded_obj["model"])
