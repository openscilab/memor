...
def test_model1():
    ...
    assert response.model == LLMModel.OpenAI.GPT_4.value


def test_model2():
    response = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4)
    response.update_model(LLMModel.OpenAI.GPT_4O)
    assert response.model == LLMModel.OpenAI.GPT_4O.value
...
def test_model4():
    response = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4)
    with pytest.raises(MemorValidationError, match=r"Invalid model. It must be an instance of LLMModel.<provider> enum."):
...
def test_json4():
    response = Response()
    with pytest.raises(MemorValidationError, match="Invalid value. `temperature` must be a non-negative number."):
        response.from_json(r"""{
                           "message": "I am fine.",
                           "type": "Response",
                           "score": 0.8,
...
def test_json5():
    response = Response()
    with pytest.raises(MemorValidationError, match="Invalid value. `tokens` must be a non-negative integer."):
        response.from_json(r"""{
                           "message": "I am fine.",
                           "type": "Response",
                           "score": 0.8,
...
def test_json6():
    response = Response()
    with pytest.raises(MemorValidationError, match="Invalid value. `inference_time` must be a non-negative number."):
        response.from_json(r"""{
                           "message": "I am fine.",
                           "type": "Response",
                           "score": 0.8,
...
def test_json7():
    response = Response()
    with pytest.raises(MemorValidationError, match="Invalid value. `top_k` must be a non-negative integer."):
        response.from_json(r"""{
                           "message": "I am fine.",
                           "type": "Response",
                           "score": 0.8,
...
def test_save1():
    response = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4, temperature=0.5, role=Role.USER, score=0.8)
...
def test_load1():
    response1 = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4, temperature=0.5, role=Role.USER, score=0.8)
...
def test_equality2():
    response1 = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4, temperature=0.5, role=Role.USER, score=0.8)
    response2 = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4, temperature=0.5, role=Role.USER, score=0.6)
    assert response1 != response2


def test_equality3():
    response1 = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4, temperature=0.5, role=Role.USER, score=0.8)
    response2 = Response(message="I am fine.", model=LLMModel.OpenAI.GPT_4, temperature=0.5, role=Role.USER, score=0.8)
    assert response1 == response2
